      ...
      <div className="bg-white rounded-xl shadow-2xl w-[480px] max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center gap-2 px-5 py-4 border-b border-border-subtle">
          ...
          <div className="flex-1">
            <div className="text-[14px] font-semibold text-text-primary">리포팅 초안 생성</div>
            <div className="text-[11px] text-text-tertiary">포함할 셀을 선택하세요</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
          {cells.map((cell) => (
            ...
            >
              ...
              <span className="text-[13px] text-text-primary flex-1">{cell.name}</span>
              {!cell.executed && (
                <span className="text-[10px] text-warning shrink-0">⚠ 실행되지 않음</span>
              )}
            </label>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-border-subtle">
          ...
          <button
            ...
            onClick={() => generateReport(selectedIds)}
            className={cn(
