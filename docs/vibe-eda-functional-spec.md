...
- **빈 상태**:
  - 검색어 있음: 자동으로 **히든룰 — Snowflake 전체 검색**으로 폴백 (TMH-006-2-2 참고). 결과도 없으면 `"{query}"에 맞는 마트가 없어요`
...
- **[TMH-006-2-1] 마트 카드**
  ...
  - **확장 마트**: `mart.extra === true` 인 항목엔 "확장" 배지 (피치 톤). 히든룰 검색으로 노트북에 추가된 마트임을 표시.

#### [TMH-006-2-2] 히든룰 — Snowflake 전체 확장 검색
- **트리거**: 마트 검색어가 ≥2자 + 로컬 매칭 0개일 때 350ms 디바운스 후 자동 호출.
- **API**: `GET /marts/search?q=…&limit=20` (`ACCOUNT_USAGE.TABLES` → 폴백 `INFORMATION_SCHEMA.TABLES`).
...
#### [MODAL-001-5] 에이전트 대화 선택 (옵션)
- 셀 리스트 아래 별도 섹션. 현재 진행 중 세션 + 아카이브 세션이 체크박스로 노출된다.
- 메시지 0개인 세션은 자동 제외.
- **기본값**: 현재 진행 중 세션만 체크 (아카이브는 명시적으로 추가).
- 현재 세션은 "현재" 배지로 구분.
- 선택된 세션은 user/assistant 메시지만 추출되어 리포트 프롬프트의 "## 에이전트 분석 대화" 섹션으로 주입 (수치 인용 금지, 가설·맥락 참고용).
