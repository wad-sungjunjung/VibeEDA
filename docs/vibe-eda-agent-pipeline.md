...
└─────────────────────────────────────────────────────────────────────────┘
  ...
  budget 5/10                    budget 25/60               budget 100/250
       ...
       │  + 메서드별 추가 필수 필드 (causal_design / ml_design /   │
       │    forecast_spec / ab_design — 누락 시 plan_field_missing) │
...
## 4-1. 메서드별 플래닝 스키마 (`create_plan`)

플래닝의 본질이 메서드마다 다르다. 모든 메서드는 공통 가설(min 3개)을 유지하되,
선택된 메서드에 따라 추가 필드가 **필수**로 강제된다 (누락 시 `plan_field_missing` 거부).

| 메서드 | 추가 필드 | 필수 서브키 | 플래닝 성격 |
|---|---|---|---|
| `analyze` | — | — | 가설 검증 (관찰형 비교) |
| `benchmark` | — | — | 가설 검증 (기준 대비) |
| `explore` | `exploration_plan` (선택) | marts_to_profile, checks | 탐색 체크리스트 |
| `predict` | `forecast_spec` | time_column, target_metric, horizon | 예측 명세 |
| `causal` | `causal_design` | treatment, outcome, confounders(≥2), assignment | 변수 선언 |
| `ml` | `ml_design` | target, task_type | 모델 설계 |
| `ab_test` | `ab_design` | control_group, treatment_group, primary_metric, mde | 통계 설계 |

복수 메서드 선택 시 (예: `causal+predict`) 두 필드 모두 필수.
검증 로직: `agent_skills._validate_method_plan_fields` (필수 필드 + 서브키 + causal confounders 최소 2개).

플랜 셀에는 가설 위에 메서드별 설계 섹션이 자동 렌더링됨:
- `causal_design` → 처치/결과/교란/배정 표 (관찰 데이터면 인과 표현 약화 경고)
- `ml_design` → 타깃/태스크/평가지표/피처 후보
- `forecast_spec` → 시계열 컬럼/학습 구간/예측 지평/계절성 표
- `ab_design` → 그룹/메트릭/MDE/power/alpha 표
- `exploration_plan` → 마트 profile 순서 + 체크리스트

시스템 프롬프트에는 `agent_methods.build_plan_field_hint_block`이 메서드별 필수 필드 안내를
`methods_block` 끝에 자동으로 덧붙인다.
...
| # | 스킬 | 메커니즘 |
|---|---|---|
| 1 | planning | `create_plan` tool + pre-guard (가설 3+ 강제 + 메서드별 추가 필드 강제) |
