...
리포트 모달에서 입력하는 "추가 코멘트" 는 `extra_comment` 로 전달되어 `context["extra_comment"]` 에 들어간다.
- 4000자 컷오프 (보통 짧음).
- outline + writing 프롬프트 양쪽에 "## 분석가 추가 코멘트 (작성 지침)" 으로 주입.
- 강조 사항·톤·생략·해석 가이드 등 작성 방향만 영향. 수치는 셀 출력에서 인용하라는 가드를 함께 포함.
