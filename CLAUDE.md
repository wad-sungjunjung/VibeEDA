...
| Agent Mode | **기본 Claude Opus** (`claude-opus-4-7`) 또는 Gemini. 프론트 `X-Agent-Model` 헤더로 스위치. **시니어 분석가 v0.5** — 4-Phase 아키텍처 (복잡도 분류→메서드 라우팅→실행→종합 정리), 3-Tier 예산 (L1/L2/L3, auto-promotion), 7개 메서드 (analyze/explore/predict/causal/ml/ab_test/benchmark) 및 메서드별 도구 (ML·Causal·Predict 각 3개) + 가드. Tool 총 34개. 노트북별 `learnings.md` 누적. **안정성 가드**: tool loop 절대 50회 캡, LLM API 호출당 5분 타임아웃, 셀 실행 1시간 (SQL/Python 동일). 자세한 내부는 `docs/vibe-eda-agent-pipeline.md` (구조도 포함). |
