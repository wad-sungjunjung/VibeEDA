...
### 7. GROUP BY가 있으면 반드시 GROUPING SETS로 작성한다
일반 GROUP BY는 절대 사용하지 않는다.
- 소계 레이블: case when grouping(col) = 1 then '0. 전체' else col end 패턴
- 정렬: order by all
...
올바른 예 (반드시 이 형태로 작성):
    with fact_entry as (
        ...
        select
            case when grouping(reg_date) = 1 then '0. 전체' else reg_date end as reg_date,
            case when grouping(channel_type) = 1 then '0. 전체' else channel_type end as channel_type,
            count(entry_id) as "입장건수"
        from fact_entry
        group by grouping sets (
            (reg_date, channel_type),
            (reg_date),
            ()
        )
