...
- **사용자가 메시지 또는 예시 쿼리에서 직접 명시한 외부 테이블** (워크스페이스 테이블, TMP 테이블, RAW 레이어 등) 은 **사용자가 제공한 전체 경로 그대로** 사용할 것
  - 예: 사용자 예시에 `WAD_DW.WORKSPACE_BA.TMP_xxx` 가 있으면 그대로 JOIN
  - 추측으로 이름을 바꾸거나 임의의 경로를 조합하지 말 것
- 사용자가 전혀 언급하지 않은 다른 스키마/DB 는 추측해서 참조 금지 — 필요한 데이터가 없으면 주석으로 `-- ⚠️ 이 분석에 필요한 마트가 선택되지 않음` 을 남기고 중단
